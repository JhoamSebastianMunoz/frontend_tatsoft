...
const Devoluciones = () => {
  ...
  const totalDevoluciones = devoluciones.reduce((total, devolucion) => {
    return total + parseInt(devolucion.total.replace(/,/g, ""), 10);
  }, 0);

  const totalFormateado = totalDevoluciones.toLocaleString();
  const devolucionesFiltradas = devoluciones.filter((devolucion) => {
    return (
      (filtro === "Todos" || devolucion.zona === filtro) &&
      (devolucion.cliente.toLowerCase().includes(busqueda.toLowerCase()) ||
        devolucion.numeroPedido.includes(busqueda))
    );
  });

  const handleVer = (devolucion) => {
    console.log("Viendo devolución:", devolucion);
  };

  const handleEditar = (devolucion) => {
    console.log("Editando devolución:", devolucion);
  };

  const handleNuevaDevolucion = () => {
    navigate("/nueva/devolucion");
  };
              ...
              <div>
                <div className="flex items-center mt-1">
                  <span className="bg-green-200 text-green-800 text-xs font-medium px-3 py-0.5 rounded-full mr-3">
                    {devoluciones.length} Total
                  </span>
                  <span className="bg-purple-200 text-purple-800 text-xs font-medium px-3 py-0.5 rounded-full">
                    {devolucionesFiltradas.length} Filtrados
                  </span>
                </div>
              </div>
            </div>
          </div>
          <div className="max-w-2xl mx-auto bg-white rounded-xl shadow-lg p-5 mb-6 border-l-4 border-purple-500">
            <div className="flex justify-between items-center mb-4">
              <div className="flex items-center">
                <h2 className="text-lg font-semibold text-gray-800">Resumen de Devoluciones</h2>
              </div>
              <button
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between py-2">
              <div className="text-center sm:text-left mb-4 sm:mb-0">
                <p className="text-gray-600 font-semibold text-sm">Monto total de devoluciones</p>
                <p className="text-3xl font-bold text-purple-700">$ {totalFormateado}</p>
              </div>
              <div className="grid grid-cols-2 gap-6">
                <div className="text-center bg-pur´ple-50 px-4 py-2 rounded-lg">
                  <p className="text-black text-xs mb-1">Total pedidos</p>
                  <p className="text-lg font-semibold text-gray-800">{devoluciones.length}</p>
                </div>
                <div className="text-center bg-purple-50 px-4 py-2 rounded-lg">
                  <p className="text-black text-xs mb-1">Mes actual</p>
                  <p className="text-lg font-semibold text-gray-800">Febrero</p>
                </div>
              </div>
            </div>
          </div>
            ...
            <div className="border-b pb-3 mb-4 flex justify-between items-center">
              <h3 className="font-medium text-black-900">
                ...
                <span className="ml-2 text-sm font-normal text-black-700">
                  Mostrando {devolucionesFiltradas.length} de {devoluciones.length}
                </span>
              </h3>
            </div>
                ...
                {devolucionesFiltradas.length > 0 ? (
                    ...
                    >
                      ...
                      <div className="bg-gray-100 flex items-center justify-between px-3 py-1 ">
                        <div className="flex items-center space-x-1">
                          ...
                          <span className="text-xs">{devolucion.fecha}</span>
                        </div>
                      </div>
                      <div className="p-4">
                        <div className="mb-2">
                          <div className="flex justify-between items-center">
                            ...
                            <p className="text-sm text-semibold text-purple-800">
                              {devolucion.numeroPedido}
                            </p>
                          </div>
                        </div>
                        <div className="mb-2">
                          <p className="text-sm font-medium">Cliente:</p>
                          <p className="text-sm">{devolucion.cliente}</p>
                        </div>
                        <div className="mb-2">
                      ...
                      <div className="px-0 bg-gray-50 border-t border-gray-100">
                        ...
                        <div className="card-buttons-container flex justify-end items-center px-1">
                          <Boton
                            onClick={() => handleEditar(devolucion)}
                            label="Ver detalle"
                            tipo="secundario"
                            size="small"
                          />
                  ...
                  <div className="col-span-full py-8 flex flex-col items-center justify-center text-center">
                    ...
                    <p className="text-gray-500">
                      No se encontraron devoluciones que coincidan con los
                        ...
                        <tr key={index} className="hover:bg-gray-50">
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm">
                            <div className="flex justify-end gap-2">
                              <Boton
                                onClick={() => handleVer(devolucion)}
                                label="Ver detalle"
                                size="small"
                                tipo="secundario"
                              />
                              <Boton
                                onClick={() => handleEliminar(devolucion)}
                                label="Eliminar"
                                tipo="cancelar"
                                size="small"
                              />
